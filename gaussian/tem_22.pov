...
cylinder {
  ...
  interior{ //-----------
    media{
      emission <0,0,0.3>
      density {
	function{ I_0*pow( w_0/w_z(x,y,z), 2 )*exp( -2*r_squared(x,y,z)/pow( w_z(x,y,z), 2 ) )*4*pow(y,2)*4*pow(x,2)}	
      }
    } // end of media ---
  } // end of interior
  translate +2*y
...
cylinder {
  ...
  interior{ //-----------
    media{
      emission <0,0,0.3>
      density {
	function{ I_0*pow( w_0/w_z(x,y,z), 2 )*exp( -2*r_squared(x,y,z)/pow( w_z(x,y,z), 2 ) )*pow(sin(-k*(z+r_squared(x,y,z)/(2*R_z(x,y,z)))+guoy_phase(x,y,z)),2)*(4*pow(y,2))*4*pow(x,2) }
      }
    } // end of media ---
  } // end of interior
  translate -2*y
